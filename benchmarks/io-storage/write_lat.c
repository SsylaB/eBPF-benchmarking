...
static void *cpu_monitor(void *arg)
{
    ...
    unsigned long long prev_ns         = 0; /* track previous monotonic timestamp */

    int self_pid = getpid();

    /* use CLOCK_MONOTONIC for baseline_end_ns to match now_ns clock */
    ...
    while (running) {
        ...
        /* elapsed wall time between samples, default 0.2s on first iteration */
        double elapsed_s = (prev_ns > 0) ? (now_ns - prev_ns) / 1e9 : 0.2;
        prev_ns = now_ns;

        /* Safety fallback: switch phase if SIGUSR1 never arrived */
        if (baseline_secs > 0 && current_phase == 0 && now_ns >= baseline_end_ns)
            current_phase = 1;

        /* Use CLOCK_REALTIME timestamp for CSV output (aligned with latency events) */
        struct timespec rts;
        clock_gettime(CLOCK_REALTIME, &rts);
        unsigned long long csv_ns = (unsigned long long)rts.tv_sec  * 1000000000ULL
        ...
        if (self_prev_utime > 0) {
            /* divide by elapsed_s to normalize against actual sample interval */
        ...
        if (filter_pid > 0) {
            ...
            if (ret < 0) {
                ...
                if (filt_prev_utime > 0) {
                    /* same elapsed_s normalization for fio CPU */
...
int main(int argc, char **argv)
{
    ...
    while (running) {
        err = ring_buffer__poll(rb, 10); /* 10ms instead of 100ms */
        if (err == -EINTR) { err = 0; break; }
        if (err < 0 && err != -ENOSPC) { /* ignore ring buffer overflow */
